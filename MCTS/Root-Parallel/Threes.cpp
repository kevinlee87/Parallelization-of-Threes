...
unsigned long long run(Board board, int dir, bool bag[3])
{
    Board tmp;
    int move[4];
    //std::cout << dir << "\n" << board << "\n";
...
void simulate(Node *s)
{
    ...
    for(int i = 0 ; i < s->num_next ; i++)
    {
        if(s->next[i])
        {
            ...
            {
                bag[0] = bag[1] = bag[2] = 1;
                s->next[i]->score += run(s->next[i]->board, s->next[i]->dir, bag);
            }
        }
    }
}
...
{
    int ans[4] = {0, 0, 0, 0}, dir = -1;
    target = board;
    for(int i = 0 ; i < nthread ; i++) pthread_create(&thread[i], NULL, MCTS, (void*)&ret[i]);
    for(int i = 0 ; i < nthread ; i++)
    {
        pthread_join(thread[i], NULL);
        if(ret[i] == -1) continue;
        ans[ret[i]]++;
        if(dir == -1 || ans[ret[i]] > ans[dir]) dir = ret[i];
    }
